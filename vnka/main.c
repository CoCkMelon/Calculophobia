...
static vk_error init_texture(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_render_essentials *essentials,
							 struct vk_image *image, const char *name)
{
...
static void free_render_data(struct vk_device *dev, struct vk_render_essentials *essentials, struct render_data *render_data)
{
	...
	vk_free_images(dev, render_data->images, 2);
...
#include "subprojects/Simple-SDL2-Audio/src/audio.c"
					...
					struct vk_swapchain *swapchain, struct app_os_window *os_window, uint32_t thread_count, VkPresentModeKHR *present_modestruct)
{
	...
	if(os_window->app_data.iMouse_click[0]>0&&up) {
		...
		vk_free_layouts(dev, &render_data.main_layout, 1);
		vk_free_shaders(dev, render_data.shaders, 2);
		//render_data.shaders[SHADER_MAIN_FRAGMENT]=(struct vk_shader){vn.slidenames[vn.slide], VK_SHADER_STAGE_FRAGMENT_BIT};
		//res = vk_load_shaders(dev, render_data.shaders, 2);
		//if (vk_error_is_error(&res)) {
